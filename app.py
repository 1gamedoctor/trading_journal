...
# ── Ensure data dirs exist (use /tmp on Cloud, local otherwise) ──────────────
import tempfile
DATA_DIR = Path(os.environ.get("TRADEFORGE_DATA_DIR", "data"))
try:
    ...
    DATA_DIR.mkdir(exist_ok=True)
os.environ["TRADEFORGE_DATA_DIR"] = str(DATA_DIR)
...
with st.sidebar:
    ...
    try:
        import json
        from pages.settings import SETTINGS_FILE
        if SETTINGS_FILE.exists():
            with open(SETTINGS_FILE) as f:
                _s = json.load(f)
            daily_limit = _s.get("daily_trade_limit", 5)
            max_daily_loss = _s.get("max_daily_loss_pct", 3.0)
            starting_balance = _s.get("starting_balance", 10000)
            ...
            if today_count >= daily_limit:
                ...
                <div style="background:rgba(255,71,87,0.1);border:1px solid rgba(255,71,87,0.3);
                    ...
                    <div style="font-size:0.78rem;color:#ff4757;">⛔ Daily limit reached ({today_count}/{daily_limit})</div>
                </div>
                """, unsafe_allow_html=True)

            daily_loss_limit = starting_balance * max_daily_loss / 100
            if today_pnl < 0 and daily_loss_limit > 0 and abs(today_pnl) >= daily_loss_limit * 0.8:
